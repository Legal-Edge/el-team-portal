...
import { blockUserAction } from '@/lib/actions/users'
...
export function inferPortalRole(title: string | null): { label: PortalRole; color: string } | null {
  ...
  if (t === 'bot' || t.includes('integration') || t.includes('leads') || (t.includes('admin') && t.includes('tech')))
...
// ── Sentinel for blank/null filters ──────────────────────────────────────────
...
export function UsersTable({ users: initialUsers }: { users: AzureUser[] }) {
  // Filter out already-blocked users on mount; removal syncs instantly
  ...
  const roles = useMemo(() => {
    ...
    users.forEach(u => {
      ...
      set.add(r ? r.label : '(No Role)')
    })
    return Array.from(set).sort().map(v => ({ value: v === '(No Role)' ? BLANK : v, label: v }))
  }, [users])
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter(u => {
      ...
      if (titleF) {
        if (titleF === BLANK ? u.title !== null : u.title !== titleF) return false
      }
      if (deptF) {
        if (deptF === BLANK ? u.department !== null : u.department !== deptF) return false
      }
      if (roleF) {
        const r = inferPortalRole(u.title)
        const label = r?.label ?? null
        if (roleF === BLANK ? label !== null : label !== roleF) return false
      }
  ...
  function onF(setter: (v: string) => void) { return (e: React.ChangeEvent<HTMLSelectElement>) => { setter(e.target.value); setPage(1) } }
      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-100 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Select value={titleF} onChange={onF(setTitleF)} placeholder="All Job Titles"   opts={titles} />
            <Select value={deptF}  onChange={onF(setDeptF)}  placeholder="All Departments"  opts={depts}  />
            <Select value={roleF}  onChange={onF(setRoleF)}  placeholder="All Portal Roles" opts={roles}  />
            {hasFilters && (
                  ...
                  <tr key={u.email} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <button onClick={() => setDeleteUser(u)}
                        className="text-gray-300 hover:text-red-400 transition-colors" title="Remove portal access">
